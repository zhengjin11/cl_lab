...
class BasicNet_exp1(nn.Module):
    ...
    def forward(self, x):
        ...
        if aux_logits is not None and old_cls_logits is not None:
            ...
            mix_p_vec = None
        return {'feature': features, 'logit': logits, 'aux_logit': aux_logits, 'old_cls_logit': old_cls_logits,'mix_p_vec': mix_p_vec}
...
class BasicNet_exp4(nn.Module):
    def __init__(
        ...
        super(BasicNet_exp4, self).__init__()
        self.nf = nf
        self.init = init
        self.convnet_type = convnet_type
        self.dataset = dataset
    ...
    def forward(self, x):
        ...
        if self.der:
            if len(self.exp_layer)==0:
                ...
                for task_id in range(self.ntask):
                    if task_id == 0:
                        x = self.convnet.conv1(input)
                        x = self.convnet.layer1(x)
                        x = self.convnet.layer2(x)
                        x = self.convnet.layer3(x)
                    else:
                        x = self.exp_layer[task_id - 1](input)
    ...
    def features_dim(self):
        if self.der:
            return self.out_dim * (len(self.exp_layer) + 1)
        else:
            return self.out_dim
    ...
    def add_classes(self, n_classes):
        self.ntask += 1
        print(self.convnet.layer4.module_list)
    ...
    def _add_classes_multi_fc(self, n_classes):
        ...
        fc = self._gen_classifier(self.out_dim * (len(self.exp_layer) + 1), self.n_classes + n_classes)

        if self.classifier is not None and self.reuse_oldfc:
            fc.weight.data[:self.n_classes, :self.out_dim * (len(self.exp_layer))] = weight
...
class BasicNet_exp5(nn.Module):
    def __init__(
        ...
        super(BasicNet_exp5, self).__init__()
        self.nf = nf
        self.init = init
        self.convnet_type = convnet_type
        self.dataset = dataset
    ...
    def add_classes(self, n_classes):
        self.ntask += 1
        print(self.convnet.layer3.module_list)
