...
class ResNet_exp4(nn.Module):
    def __init__(self,
        ...
        self.layer1 = self._make_layer_ordinary(block, 1 * nf, layers[0])
        self.layer2 = self._make_layer_ordinary(block, 2 * nf, layers[1], stride=2)
        self.layer3 = self._make_layer_ordinary(block, 4 * nf, layers[2], stride=2)
        self.layer4 = self._make_layer(BasicBlock_expandable, 8 * nf, layers[3], stride=2, remove_last_relu=remove_last_relu)
...
def resnet18_exp4(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    """
    model = ResNet_exp4(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model

def resnet18_exp5(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    """
    model = ResNet_exp5(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model