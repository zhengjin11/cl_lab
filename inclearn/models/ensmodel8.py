...
class EnsModel8(IncrementalLearner):
    def __init__(self, cfg, trial_i, _run, ex, tensorboard, inc_dataset):
        ...
        self._network = network.BasicNet_ens8(
            cfg["convnet"],
            cfg=cfg,
            nf=cfg["channel"],
            device=self._device,
    ...
    def train(self):
        if self._der:
            if self._task == 0:
                ...
                if self._task == 1:
                    pass
                    # self._parallel_network.module.convnet.layer3.train()
                    # self._parallel_network.module.convnet.layer4.train()
                else:
                    # self._parallel_network.module.convnet.conv1.train()
                    # self._parallel_network.module.convnet.layer1.train()
                    # self._parallel_network.module.convnet.layer2.train()
                    # self._parallel_network.module.convnet.layer3.train()

                    for m in self._parallel_network.module.convnet.conv1.modules():
                        if isinstance(m, nn.BatchNorm2d):
                            m.train()
                    for m in self._parallel_network.module.convnet.layer1.modules():
                        if isinstance(m, nn.BatchNorm2d):
                            m.train()
                    for m in self._parallel_network.module.convnet.layer2.modules():
                        if isinstance(m, nn.BatchNorm2d):
                            m.train()
                    for m in self._parallel_network.module.convnet.layer3.modules():
                        if isinstance(m, nn.BatchNorm2d):
                            m.train()

                    # self._parallel_network.module.final_layer[-2].train()
    ...
    def set_optimizer(self, lr=None):
        ...
        if self._der and self._task > 0:
            ...
            if self._task == 1:
                for p in self._parallel_network.module.convnet.layer3.parameters():
                    p.requires_grad = False
                for p in self._parallel_network.module.convnet.layer4.parameters():
                    p.requires_grad = False
            else:
                for i in range(self._task-1):
                ...
                # for p in self._parallel_network.module.convnet.conv1.parameters():
                #     p.requires_grad = True
                # for p in self._parallel_network.module.convnet.layer1.parameters():
                #     p.requires_grad = True
                # for p in self._parallel_network.module.convnet.layer2.parameters():
                #     p.requires_grad = True
                # for p in self._parallel_network.module.convnet.layer3.parameters():
                #     p.requires_grad = True
